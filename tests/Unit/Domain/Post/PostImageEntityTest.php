...
it('画像エンティティを生成できる', function () {
    $image = new PostImage(
        id: 'uuid-img-1',
        postId: 'uuid-post-1',
        url: 'https://example.com/post_images/test.jpg',
        order: 0,
    );

    expect($image->id)->toBe('uuid-img-1')
        ->and($image->postId)->toBe('uuid-post-1')
        ->and($image->url)->toBe('https://example.com/post_images/test.jpg')
        ->and($image->order)->toBe(0);
});

it('複数の画像エンティティをorderで並べられる', function () {
    $images = [
        new PostImage(id: 'uuid-img-2', postId: 'uuid-post-1', url: 'https://example.com/2.jpg', order: 1),
        new PostImage(id: 'uuid-img-1', postId: 'uuid-post-1', url: 'https://example.com/1.jpg', order: 0),
    ];
