...
it('ハッシュタグ付き投稿を作成するとハッシュタグが保存される', function () {
    ...
    $useCase = new CreatePostUseCase($postRepository, $hashtagRepository, $imageRepository);
    $post = $useCase->execute('uuid-post-1', 'uuid-user-1', 'テストユーザー', 'test_user', '#Laravel と #PHP の話');
...
it('同じハッシュタグが複数あっても重複せずに保存される', function () {
    ...
    $useCase = new CreatePostUseCase($postRepository, $hashtagRepository, $imageRepository);
    $useCase->execute('uuid-post-1', 'uuid-user-1', 'テストユーザー', 'test_user', '#Laravel #Laravel #Laravel');
});
...
it('ハッシュタグなしの投稿ではsyncToPostは呼ばれない', function () {
    ...
    $useCase = new CreatePostUseCase($postRepository, $hashtagRepository, $imageRepository);
    $useCase->execute('uuid-post-1', 'uuid-user-1', 'テストユーザー', 'test_user', 'ハッシュタグなしの投稿');
});
